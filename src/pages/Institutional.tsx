...
export default function Institutional() {
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from("institutional-docs").upload(fileName, file, {
        cacheControl: "3600",
        upsert: true
      });
      ...
      const {
        data: urlData
      } = supabase.storage.from("institutional-docs").getPublicUrl(fileName);
      const {
        error: updateError
      } = await supabase.from("app_settings").update({
        planning_pdf_url: urlData.publicUrl
      }).eq("id", settings?.id);
  ...
  return <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Información General del Grupo GISICF</h1>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="relative">
          ...
          <CardContent className="space-y-4">
            ...
            {isAdmin && <Dialog open={missionOpen} onOpenChange={setMissionOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Textarea id="mission" value={missionText} onChange={e => setMissionText(e.target.value)} rows={6} className="mt-2" />
                    </div>
        ...
        <Card className="relative">
          ...
          <CardContent className="space-y-4">
            ...
            {isAdmin && <Dialog open={visionOpen} onOpenChange={setVisionOpen}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="w-full">
                    <Pencil className="h-4 w-4 mr-2" />
                    Editar Visión
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Textarea id="vision" value={visionText} onChange={e => setVisionText(e.target.value)} rows={6} className="mt-2" />
                    </div>
                    <Button onClick={() => updateMutation.mutate({
                  field: "vision_text",
                  value: visionText
        ...
        <Card className="relative">
          ...
          <CardContent className="space-y-4">
            ...
            {settings?.planning_pdf_url ? <Button asChild variant="default" className="w-full">
                <a href={settings.planning_pdf_url} target="_blank" rel="noopener noreferrer">
                  <Download className="h-4 w-4 mr-2" />
                  Descargar Planificación Vigente
                </a>
              </Button> : <p className="text-sm text-center text-muted-foreground">
            ...
            {isAdmin && <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input id="pdf-upload" type="file" accept=".pdf" onChange={handlePdfUpload} disabled={uploading} className="flex-1" />
